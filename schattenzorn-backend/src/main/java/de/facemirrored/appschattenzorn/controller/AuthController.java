  ...
  public ResponseEntity<SignUpResponse> registerUser(@Valid @RequestBody
    ...
    if (Boolean.TRUE.equals(userRepository.existsByEmail(signUpRequest.getEmail()))) {
      return ResponseEntity
          .ok(new SignUpResponse(Status.FAILED_EMAIL_TAKEN));
    }

    if (Boolean.TRUE
        .equals(userRepository.existsByUsername(signUpRequest.getUsername()))) {
      return ResponseEntity
          .ok(new SignUpResponse(Status.FAILED_USERNAME_TAKEN));
    }

    authService.signUpUser(signUpRequest);

    return ResponseEntity.ok(new SignUpResponse(Status.SUCCESS));
  }
}
